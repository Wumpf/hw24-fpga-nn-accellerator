...
with Image.open('giphy.gif') as img:
    ...
    for frame in ImageSequence.Iterator(img):
        keep_frame_if_zero = (keep_frame_if_zero + 1) % 3
        if keep_frame_if_zero != 0:
            continue
        frame_resized = frame.resize((64, 64))
        frame_resized = frame_resized.convert('RGB')
        frames_resized.append(frame_resized)


# Load the image
image = frames_resized[0]
...
for i in range(1, len(frames_resized)):
    rr.set_time_sequence("frame", i)
    rr.log("train_input", rr.Image(frames_resized[i]))
...
joel = 64
def gen_input(x_coords, y_coords):

    # the sin functions introduce artifacts at the bottom
    return np.array([np.array([[np.sin(x * i), np.sin(y * i)] for i in range(joel)]).flatten()
                for y in y_coords for x in x_coords])

X = gen_input(x_coords, y_coords)
...
layer_size = 64
model = Sequential([
    Dense(layer_size, input_dim=joel * 2, activation='relu'),
...
predicted_rgb = predicted_rgb.reshape((len(x_coords), len(y_coords), 3)).astype(np.uint8)
